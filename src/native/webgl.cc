    ...
    int width, int height, bool alpha, bool depth, bool stencil, bool antialias, bool premultipliedAlpha, bool preserveDrawingBuffer, bool preferLowPowerToHighPerformance, bool failIfMajorPerformanceCaveat) : state(GLCONTEXT_STATE_INIT), unpack_flip_y(false), unpack_premultiply_alpha(false), unpack_colorspace_conversion(0x9244), unpack_alignment(4), next(NULL), prev(NULL), lastError(GL_NO_ERROR)
{
  ...
  EGLint surfaceAttribs[] = {
      EGL_WIDTH, (EGLint)1, EGL_HEIGHT, (EGLint)1, EGL_NONE};
...
GL_METHOD(FramebufferTexture2D)
{
  ...
  if (attachment == 0x821A)
  {
    (inst->glFramebufferTexture2D)(
        target, GL_DEPTH_ATTACHMENT, textarget, texture, level);
    (inst->glFramebufferTexture2D)(
        target, GL_STENCIL_ATTACHMENT, textarget, texture, level);
  }
  else
  {
    (inst->glFramebufferTexture2D)(
...
GL_METHOD(FramebufferRenderbuffer)
{
  ...
  if (attachment == 0x821A)
  {
    (inst->glFramebufferRenderbuffer)(
        target, GL_DEPTH_ATTACHMENT, renderbuffertarget, renderbuffer);
    (inst->glFramebufferRenderbuffer)(
        target, GL_STENCIL_ATTACHMENT, renderbuffertarget, renderbuffer);
  }
  else
  {
    (inst->glFramebufferRenderbuffer)(
...
GL_METHOD(GetActiveAttrib)
{
  ...
  if (length > 0)
  {
    ...
    Nan::Set(activeInfo, Nan::New<v8::String>("size").ToLocalChecked(), Nan::New<v8::Integer>(size));
    Nan::Set(activeInfo, Nan::New<v8::String>("type").ToLocalChecked(), Nan::New<v8::Integer>(type));
    Nan::Set(activeInfo, Nan::New<v8::String>("name").ToLocalChecked(), Nan::New<v8::String>(name).ToLocalChecked());
...
GL_METHOD(GetShaderPrecisionFormat)
{
  ...
  Nan::Set(result, Nan::New<v8::String>("rangeMin").ToLocalChecked(), Nan::New<v8::Integer>(range[0]));
  Nan::Set(result, Nan::New<v8::String>("rangeMax").ToLocalChecked(), Nan::New<v8::Integer>(range[1]));
  Nan::Set(result, Nan::New<v8::String>("precision").ToLocalChecked(), Nan::New<v8::Integer>(precision));
